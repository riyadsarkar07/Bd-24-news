...
function CategoryDropdown({ category, onClose }: { category: Category; onClose: () => void }) {
  const { lang } = useLanguage();
  const { data: articles } = useArticles({ category: category.slug, limit: 6 });

  return (
    <div className="container-page grid grid-cols-12 gap-8 py-6">
      <div className="col-span-3">
        <p className="font-bengali text-lg font-bold" style={{ color: category.color }}>
          {category.nameBn}
        </p>
      ...
      <div className="col-span-6 grid grid-cols-3 gap-4">
        {(articles ?? []).slice(0, 3).map((a) => (
          <Link key={a.id} href={`/article/${a.slug}`} className="group block" onClick={onClose}>
      ...
      <div className="col-span-3 grid content-start gap-1">
        ...
        {(articles ?? []).slice(0, 5).map((a, i) => (
          ...
          >
            <span className="text-sm font-black" style={{ color: category.color }}>
            ...
            <div key={cat.slug} className="relative shrink-0" onMouseEnter={() => setOpen(cat.slug)}>
              ...
              <AnimatePresence>
                {open === cat.slug && (
                  ...
                  >
                    <CategoryDropdown category={cat} onClose={() => setOpen(null)} />
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
